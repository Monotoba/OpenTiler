...
class ManualOpenTilerCapture:
    ...
    def _show_status(self):
        ...
        print(f"  OpenTiler window: {self.opentiler_window.title if self.opentiler_window else 'Not found'}")
        print(f"  Output directory: {self.output_dir}")
        ...
        if png_files or jpg_files:
            ...
            all_files = sorted(png_files + jpg_files, key=lambda x: x.stat().st_mtime, reverse=True)
    ...
    def _list_windows(self):
        ...
        for i, window in enumerate(windows, 1):
            ...
            print(f"      Size: {window.width}x{window.height}, Position: ({window.left}, {window.top})")
    ...
    def _capture_screenshot(self, name: str, description: str, 
