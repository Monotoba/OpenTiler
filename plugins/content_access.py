...
class PlanViewAccess(QObject):
    ...
    def get_content_image(self, bounds: Optional[QRectF] = None, 
                         resolution: Optional[Tuple[int, int]] = None) -> Optional[QImage]:
        """
    ...
    def set_view_transform(self, zoom: Optional[float] = None, 
                          pan: Optional[Tuple[float, float]] = None,
                          rotation: Optional[float] = None) -> bool:
        """
...
class TilePreviewAccess(QObject):
    ...
    def get_tile_image(self, tile_index: int, resolution: Optional[Tuple[int, int]] = None) -> Optional[QImage]:
        """
...
class MeasurementAccess(QObject):
    ...
    def get_measurements(self) -> List[MeasurementInfo]:
        ...
        if hasattr(self.main_window, 'measurement_system'):
            measurement_data = self.main_window.measurement_system.get_all_measurements()
    ...
    def add_measurement(self, start_point: QPointF, end_point: QPointF, 
...
class ContentAccessManager(QObject):
    ...
    def grant_access(self, plugin_name: str, requirements: Dict[str, bool], 
                    access_level: AccessLevel = AccessLevel.READ_ONLY) -> Dict[str, Any]:
        """
        ...
        if requirements.get('tile_preview', False):
            access_objects['tile_preview'] = TilePreviewAccess(self.main_window, access_level)
        
        # Measurement access
        if requirements.get('measurements', False):
            access_objects['measurements'] = MeasurementAccess(self.main_window, access_level)
